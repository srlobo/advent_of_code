...
func main() {
	...
	for j := 0; j <= board.maxY; j++ {
		for i := 0; i <= board.maxX; i++ {
			tails := make(trailTails)
			c := Coord{X: i, Y: j}
			if height, ok := board.elements[c]; ok && height == 0 {
				for n := range board.findTrail(c) {
					tails[n] = empty
				}
				fmt.Println("trailHead:", c, len(tails))
				total += len(tails)
			}
		}
	}
...
func (board *Board) findTrail(c Coord) trailTails {
	...
	if newValue-actualValue == 1 {
		for n := range board.findTrail(newCoord) {
			tails[n] = empty
		}
	}
