...
func (board *Board) combineAntennas(coords []Coord) {
	for i := 0; i < len(coords); i++ {
		for j := 0; j < len(coords); j++ {
			...
			coord := getMirrorCoord(coords[i], coords[j])
			if !board.isOutside(coord) {
...
func getMirrorCoord(a, b Coord) Coord {
	newCoord := Coord{X: b.X + (b.X - a.X), Y: b.Y + (b.Y - a.Y)}
	fmt.Println("a:", a, "b:", b, "newCoord:", newCoord)
	return newCoord
}
