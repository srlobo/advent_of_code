...
func main() {
	...
	board.drawMap()
	boardBeams := make(map[Coord]byte)

	total := 0
	beams := make(map[int]struct{})
	beams[start.X] = empty
	for row := start.Y; row < board.maxY; row++ {
		currentBeams := make(map[int]struct{})
		for beam := range beams {
			coord := Coord{beam, row}
			if board.elements[coord] == '^' {
				currentBeams[beam+1] = empty
				currentBeams[beam-1] = empty
				total += 1
				boardBeams[Coord{beam + 1, row}] = '|'
				boardBeams[Coord{beam - 1, row}] = '|'
			} else {
				currentBeams[beam] = empty
				boardBeams[Coord{beam, row}] = '|'
			}
		}
		beams = currentBeams
	}
	board.drawMapWithCoords(boardBeams)
	fmt.Println(total)
}
