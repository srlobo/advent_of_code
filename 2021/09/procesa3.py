...
def get_4_around_points(x, y, matrix):
    val = matrix[y][x]
    # print(f"Calculando alrededores de ({x},{y}), valor {val}")
    for r_x in range(x - 1, x + 2):
        if r_x >= 0 and r_x < len(matrix[y]):
            if matrix[y][r_x] == 9:
                continue
            if matrix[y][r_x] >= val + 1:
                # print(f"Encontrado ({r_x},{r_y}), valor {matrix[r_y][r_x]} == {val} + 1")
                yield (r_x, y)

    for r_y in range(y - 1, y + 2):
        if r_y >= 0 and r_y < len(matrix):
            if matrix[r_y][x] == 9:
                continue
            if matrix[r_y][x] >= val + 1:
                # print(f"Encontrado ({r_y},{r_y}), valor {matriy[r_y][r_y]} == {val} + 1")
                yield (x, r_y)
...
for lp_x, lp_y in lowest_points:
    ...
    candidates.update(get_4_around_points(lp_x, lp_y, matrix))
    candidates = candidates - processed
    while candidates != set():
        new_candidates = set()
        for x, y in candidates:
            new_candidates.update(get_4_around_points(x, y, matrix))
            processed.add((x, y))
