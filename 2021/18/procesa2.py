...
def main():
    ...
    sums = []
    for a, b in itertools.permutations(exprs, 2):
        a_tree = binary_tree_constructor(a)
        b_tree = binary_tree_constructor(b)
        s = snail_sum(a_tree, b_tree)
        c = calculate_magnitude(s)
        print(f"{a} + {b} = {s} -> {c}")
        sums.append(c)
    print(max(sums))
