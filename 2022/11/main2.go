...
func main() {
	...
	for monkey_n := 0; monkey_n < len(monkeys); monkey_n++ {
		fmt.Printf("bigModulus product: %d\n", monkeys[monkey_n].test)
		bigModulus *= monkeys[monkey_n].test
	}
	fmt.Printf("bigModulus: %d\n", bigModulus)
	turn_n := 0
	...
	for {
		...
		for monkey_n := 0; monkey_n < len(monkeys); monkey_n++ {
			...
			for i := 0; i < len(monkey.items); i++ {
				...
				worryLevel = worryLevel % bigModulus
		...
		if turn_n%1000 == 0 || turn_n == 1 || turn_n == 20 {
			fmt.Printf("== After round %d ==\n", turn_n)
			for monkey_n := 0; monkey_n < len(monkeys); monkey_n++ {
				fmt.Printf("Monkey %d inspected items %d times.\n", monkey_n, monkey_inspections[monkey_n])
			}
			fmt.Println()
		}
		if turn_n == 10000 {
