...
func main() {
	...
	totalFSSize := tree.size
	totalFreeSpace := 70000000 - totalFSSize
	neededSpace := 30000000 - totalFreeSpace

	fmt.Println("------------------------")
	fmt.Printf("total: %d, free: %d, neeeded: %d\n", totalFSSize, totalFreeSpace, neededSpace)

	dirsWithSizes := tree.extractDirs()

	actual_size := 70000000
	for i := 0; i < len(dirsWithSizes); i++ {
		actual_dir := dirsWithSizes[i]
		fmt.Printf("%s -> %d\n", actual_dir.path, actual_dir.size)
		if actual_dir.size >= neededSpace && actual_dir.size < actual_size {
			actual_size = actual_dir.size
		}
	}

	fmt.Println(actual_size)
...
func (tree *treeNode) extractDirs() []*treeNode {
	ret := []*treeNode{}
	if tree.t == "d" {
		ret = append(ret, tree)
		for _, v := range tree.children {
			ret = append(ret, v.extractDirs()...)
		}
	}
	return ret
}
